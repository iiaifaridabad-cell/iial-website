...
import {
  CheckCircle2, Clock, Award, Users, Zap, Brain,
...
export default function Enroll() {
  ...
  const mutation = useMutation({
    mutationFn: (data: EnrollFormData) => apiRequest("POST", "/api/enroll", data),
    onSuccess: () => {
      toast({
      ...
      <section className="py-20 bg-white dark:bg-gray-950">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid lg:grid-cols-2 gap-12 items-start">
            <div>
              <Badge className="mb-4" variant="secondary">Apply Now</Badge>
              <h2 className="text-2xl font-bold text-foreground mb-2">Complete Your Enrollment</h2>
              <p className="text-muted-foreground mb-6 text-sm">Fill in your details and we'll contact you within 24 hours to confirm your spot.</p>
              ...
              <Form {...form}>
                <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-5">
                  ...
                  {mutation.isSuccess && (
                    <div data-testid="enroll-success" className="flex items-center gap-3 p-4 rounded-xl bg-emerald-50 dark:bg-emerald-950/30 border border-emerald-200 dark:border-emerald-800">
                      <CheckCircle2 className="w-5 h-5 text-emerald-600 flex-shrink-0" />
                      <div>
                        <p className="text-sm font-semibold text-emerald-700 dark:text-emerald-400">Enrollment submitted successfully!</p>
                        <p className="text-xs text-emerald-600 dark:text-emerald-500">Our team will contact you within 24 hours to confirm your spot and send session details.</p>
                      </div>
                    </div>
                  )}

                  <p className="text-xs text-muted-foreground text-center">
                    By enrolling, you agree to our Terms of Service and Privacy Policy. We'll never share your information.
                  </p>
                </form>
              </Form>
            </div>
            ...
            <div className="space-y-6">
              <div>
                <Badge className="mb-4" variant="secondary">What's Included</Badge>
                <h2 className="text-2xl font-bold text-foreground mb-5">Everything in One Session</h2>
              </div>

              <div className="grid grid-cols-2 gap-3">
                {benefits.map((b, idx) => (
                  <div key={b.text} data-testid={`benefit-${idx}`} className="flex items-center gap-2.5 p-3 rounded-lg border border-border bg-card">
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-foreground italic leading-relaxed mb-2">
                        "I enrolled as a fresh graduate with zero AI experience. 90 minutes later I had skills on my resume that got me hired within 3 weeks."
                      </p>
                      <p className="text-xs text-muted-foreground">Rahul Mehta — Business Graduate, Placed at TechVentures</p>
                    </div>
                  </div>
                </CardContent>
              </Card>
